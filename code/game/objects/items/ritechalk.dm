...
/obj/item/ritechalk/attack_self(mob/living/user)
	...
	switch(runeselection)
		...
		if("Rune of Death")
			to_chat(user,span_cultsmall("I begin inscribing the rune of His Embrace..."))
			if(do_after(user, 30, src))
				playsound(src, 'sound/foley/scribble.ogg', 40, TRUE)
				new /obj/structure/ritualcircle/necra(step_turf)
		if("Rune of Plague")
			to_chat(user,span_cultsmall("I begin inscribing the rune of His Plague..."))
			if(do_after(user, 30, src))
				playsound(src, 'sound/foley/scribble.ogg', 40, TRUE)
		...
		if("Rune of ZIZO")
			to_chat(user,span_cultsmall("I begin inscribing the rune of His Knowledge..."))
			if(do_after(user, 30, src))
				playsound(src, 'sound/foley/scribble.ogg', 40, TRUE)
